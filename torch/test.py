...
def objective(trial):
    ...
    activate = trial.suggest_categorical("non_linear_activation", ["Relu", "PReLU","LeakyReLU","RReLU"])
    ...
    for _fold, index in enumerate(kf.split(train,train['action'])):
        ...
        for epoch in range(EPOCHS):
            ...
            valid_logloss = log_loss(valid[target_cols].values, valid_pred,sample_weight=valid['weight'].values)
            ...
            es(valid_logloss, model, model_path=model_weights)
            if es.early_stop:
    ...
    if True:
        ...
        test_loader = DataLoader(test_set, batch_size=BATCH_SIZE, shuffle=True, num_workers=100)
...
def train_fn(model, optimizer, scheduler, loss_fn, dataloader, device):
    ...
    for data in dataloader:
        ...
        loss = loss_fn(outputs, label, weight)
        loss.backward()
        optimizer.step()
        if scheduler:
            scheduler.step()
...
def utility_score_bincount(date, weight, resp, action):
    ...
    print('data_points:',len(date))
    print('action_rate:',(action).mean())
    print('total_dates:',len(set(date)))
    print('total_profit:',np.sum(weight * resp * action))
